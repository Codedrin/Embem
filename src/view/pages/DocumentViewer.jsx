        ...
        const fetchFile = async () => {
            try {
                ...
                if (bt_file) {
                    ...
                    if (downloadedFile) {
                        const fileBlob = new Blob([downloadedFile], { type: 'application/pdf' });
                ...
                console.error(error);
                toast.error(error.message || FILE_RETRIEVE_ERROR_MESSAGE);
            }
        };
        ...
        <main className="w-[100%] h-[100vh]">
            <section className="h-[5%] bg-primary-500 flex items-center">
                <button className="flex items-center p-4 gap-3" onClick={handleBackBtn}>
                    ...
                    <span>
                        <h1 className="font-bold text-white">Choose another file</h1>
                    </span>
                </button>
            </section>
            <section className="h-[90%] flex overflow-auto">
                <div className="w-[50%] p-2 overflow-auto">
                    {file && <DocumentView file={file} onLoad={handleOnLoadDocument} />}
                </div>
                <div className="w-[50%] flex justify-center items-center">
                        ...
                        <div className="ml-5 flex flex-col gap-1 mt-2">
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    name="pages-options"
                                    id=""
                                    checked={printMethod === SPECIFIC_ONLY}
                                    onChange={handleChangePrintMethod}
                                    value={SPECIFIC_ONLY}
                                    className="text-primary-500"
                                />
                            ...
                            <div className="flex flex-row gap-3 ml-6 mt-2">
                                <div>
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        disabled={printMethod !== SPECIFIC_ONLY}
                                        value={fromPages}
                                        max={maxPages}
                                        min={1}
                                        onChange={(e) => setFromPages(e.target.value)}
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        max={maxPages}
                                        min={1}
                                        onChange={(e) => setToPages(e.target.value)}
                                    />
                                </div>
                            </div>
                        </div>
                    ...
                    name="print-options"
                    id=""
                    checked={printStyle === COLORED}
                    className="text-primary-500"
                    onChange={handleChangePrintStyle}
                    value={COLORED}
                    />
                    <div>
                    ...
                    <input
                    className="border px-3 py-2 rounded-md text-sm focus
                    w-24 mt-2"
                    type="number"
                    value={copies}
